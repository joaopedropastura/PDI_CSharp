...
(Bitmap bmp, float[] img) affine((Bitmap bmp, float[] img) t, params float[] p)
{
    ...
    for (int i = 0; i < hei; i++)
    {
        ...
        {
            var index = i + j *wid;
            var x = (int)((p[0]*i) + (p[1]*j) + p[2]);
            var y = (int)((p[3]*i) + (p[4]*j) + p[5]);


            int newIndex = x + y *wid;
            if (x < 0 || y < 0 || x >= wid || y >= hei)
                continue;
            // if(newIndex < _img.Length-1 && newIndex > 0)
...
(Bitmap bmp, float[] img) sobel((Bitmap bmp, float[] img) t)
{
    ...
    float sum = 0;
    float[] result = new float[_img.Length];
    float[] result3 = new float[_img.Length];
    for (int i = 1; i < hei -1; i++)
    {
        for (int j = 1; j < wid -1; j++)
        {
            index = i + j * wid;
            sum = (_img[(index) -1] + _img[index]) - (_img[index] + _img[(index) +1]);
            result[index] = sum;
        }
    }
    ...
    for (int j = 1; j < wid -1; j++)
    {
        ...
        {
            index = i + j * wid;
            sum = result[index - 1] + 2 * result[index] + result[index +1];
            result3[index] = sum;

            if (result3[index] > 1f)
                result3[index] = 1f;
            else if (result3[index] < 0f)
                result3[index] = 0f;
        }
    }
    return (t.bmp, result3);
}
...
image = affine(image, .6f, -.8f, 170f, .8f, .6f, -50f);
show(image);