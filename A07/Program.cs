...
(Bitmap bmp, float[] img) bilinear((Bitmap bmp, float[] img) org)
{
    ...
    for (int j = 1; j < hei; j++)
    {
        for (int i = 1; i < wid; i++)
        {
            ...
            while(orgImg[c1Index] == 0f || orgImg[c2Index] == 0f || orgImg[c3Index] == 0f || orgImg[c4Index] == 0f)
            {
                if(flag)
                {
                    if((c1Index > 0 && c2Index < wid - 1) || (c3Index > 0 && c4Index < wid - 1) || ((c3Index/wid) < hei - 1 && (c4Index/wid) < hei - 1))
                    {
                        ...
                        else if (orgImg[c3Index] == 0f || orgImg[c4Index] == 0f)
                        {
                            c3Index += wid;
                            c4Index += wid;
                        }
                ...
                else
                {
                    if((c1Index > 0 && c2Index < wid - 1) || (c3Index > 0 && c4Index < wid - 1)|| ((c3Index/wid) < hei - 1 && (c4Index/wid) < hei - 1))
                    {
                        ...
                        else if (orgImg[c2Index] == 0f || orgImg[c4Index] == 0f)
                        {
                            c2Index += 1;
                            c4Index += 1;
                        }
...
(Bitmap bmp, float[] img) openRGB(string path)
{
    var bmp = Bitmap.FromFile(path) as Bitmap;
    var byteArray = bytes(bmp);
    var dataCont = continuous(byteArray);
